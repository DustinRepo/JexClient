...
public class SpaceSpeed extends FeatureExtension {

    public SpaceSpeed() {
        super(Speed.Mode.SPACE, Speed.class);
    }
    ...
    public void pass(Event event) {
        if (event instanceof EventMove eventMove) {
            ...
            if (PathingHelper.INSTANCE.isPathing()) {
                eventMove.setX(eventMove.getX() * speed.spaceSpeedProperty.value());
                eventMove.setZ(eventMove.getZ() * speed.spaceSpeedProperty.value());
            } else
                PlayerHelper.INSTANCE.setMoveSpeed(eventMove, speed.spaceSpeedProperty.value());
        }
    }
