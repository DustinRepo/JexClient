...
public final Property<Boolean> otherProperty = new Property.PropertyBuilder<Boolean>(this.getClass())
    ...
    public BlockPos getCrop() {
	 ...
	 if(!(block instanceof Fertilizable) && block instanceof GrassBlock){
		...
		if (block instanceof BambooBlock bambooBlock | block instanceof BambooSaplingBlock) {
                int age = Wrapper.INSTANCE.getWorld().getBlockState(blockPos).get(bambooBlock.AGE());
                    if (age < 1)
                      return blockPos;
                    }
		}		
