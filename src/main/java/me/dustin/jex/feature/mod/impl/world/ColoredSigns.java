...
public class ColoredSigns extends Feature {
    ...
    private final EventListener<EventDrawScreen> eventDrawScreenEventListener = new EventListener<>(event -> {
        ...
        Render2DHelper.INSTANCE.fillAndBorder(event.getMatrixStack(), width / 2 - 202, 5, width / 2 - 200 + FontHelper.INSTANCE.getStringWidth("Color Cheat Sheet") + 4, 236, 0xffffffff, 0x60000000, 1);
        FontHelper.INSTANCE.drawWithShadow(event.getMatrixStack(), "Color Cheat Sheet", width / 2 - 200, 7, -1);
        int count = 0;
        for (Formatting value : Formatting.values()) {
            String first = value.getCode() == 'k' ? "" : "\247" + value.getCode();
            FontHelper.INSTANCE.drawWithShadow(event.getMatrixStack(),  first + "&" + value.getCode(), width / 2 - 200, 16 + count * 10, -1);
            count++;
        }
