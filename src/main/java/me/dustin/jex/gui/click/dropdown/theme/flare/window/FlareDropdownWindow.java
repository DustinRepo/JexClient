    ...
    public void init() {
        ...
        for (Feature feature : FeatureManager.INSTANCE.getFeatures()) {
            if (feature.getFeatureCategory().name().equalsIgnoreCase(getName())) {
                FlareDropdownFeatureButton dropdownFeatureButton = new FlareDropdownFeatureButton(this, feature, getX() + 1, getY() + getTheme().getTopBarSize() + getTheme().getTopBarOffset() + (i * (getTheme().getButtonSize() + getTheme().getButtonOffset())), getWidth() - 2, getTheme().getButtonSize());
