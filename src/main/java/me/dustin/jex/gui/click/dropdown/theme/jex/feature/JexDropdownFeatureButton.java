...
public class JexDropdownFeatureButton extends DropdownFeatureButton {
    ...
    public void render(MatrixStack matrixStack) {
        ...
        if (isOpen()) {
            float bottomY = this == getWindow().getButtons().get(getWindow().getButtons().size() - 1) ? getY() + getHeight() + getWindow().getTheme().getButtonSize() : getWindow().getButtons().get(getWindow().getButtons().indexOf(this) + 1).getY();
