...
public class JexPluginScreen extends Screen {
    ...
    public JexPluginScreen(Screen parent) {
        super(Component.literal("Plugin Manager"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        Button cancelButton = new Button(width / 2 + 2, height - 22, 200, 20, Component.literal("Cancel"), button -> Wrapper.INSTANCE.getMinecraft().setScreen(parent));
        Button openFolderButton = new Button(width / 2 - 202, height - 22, 200, 20, Component.literal("Open Plugins Folder"), button -> Util.getPlatform().openFile(new File(ModFileHelper.INSTANCE.getJexDirectory(), "plugins")));
        enableButton = new Button(width / 2 - 202, height - 44, 200, 20, Component.literal("Enable"), button -> JexPlugin.enable(getSelected().getJexPlugin()));
        disableButton = new Button(width / 2 + 2, height - 44, 200, 20, Component.literal("Disable"), button -> JexPlugin.disable(getSelected().getJexPlugin()));
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        FontHelper.INSTANCE.draw(matrices, "Plugins: %s%d".formatted(ChatFormatting.AQUA, pluginButtons.size()), 5, 35, -1);
        ...
        Render2DHelper.INSTANCE.fill(matrices, 210, 50, width - 5, height - 50, 0x50000000);
        ...
        if (selected != null) {
            ...
            FontHelper.INSTANCE.draw(matrices, "By: %s%s".formatted(ChatFormatting.GRAY, selected.getAuthors()), 285, 66, -1);
            ...
            Wrapper.INSTANCE.getTextRenderer().drawWordWrap(Component.literal(selected.getJexPlugin().getInfo().getDescription()), 215, 125, width - 220, -1);
        }
