...
public class JexPluginScreen extends Screen {
    ...
    public JexPluginScreen(Screen parent) {
        super(Text.literal("Plugin Manager"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        ButtonWidget cancelButton = new ButtonWidget(width / 2 + 2, height - 22, 200, 20, Text.literal("Cancel"), button -> Wrapper.INSTANCE.getMinecraft().setScreen(parent));
        ButtonWidget openFolderButton = new ButtonWidget(width / 2 - 202, height - 22, 200, 20, Text.literal("Open Plugins Folder"), button -> Util.getOperatingSystem().open(new File(ModFileHelper.INSTANCE.getJexDirectory(), "plugins")));
        enableButton = new ButtonWidget(width / 2 - 202, height - 44, 200, 20, Text.literal("Enable"), button -> JexPlugin.enable(getSelected().getJexPlugin()));
        disableButton = new ButtonWidget(width / 2 + 2, height - 44, 200, 20, Text.literal("Disable"), button -> JexPlugin.disable(getSelected().getJexPlugin()));
