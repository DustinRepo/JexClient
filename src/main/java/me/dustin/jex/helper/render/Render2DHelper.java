...
public enum Render2DHelper {
    ...
    public void boxNoBeginEnd(MatrixStack matrixStack, float left, float top, float right, float bottom, int color) {
        Matrix4f matrix4f = matrixStack.peek().getModel();
        float f = (float)(color >> 24 & 255) / 255.0F;
        float g = (float)(color >> 16 & 255) / 255.0F;
        float h = (float)(color >> 8 & 255) / 255.0F;
        float k = (float)(color & 255) / 255.0F;
        BufferBuilder bufferBuilder = Tessellator.getInstance().getBuffer();
        bufferBuilder.vertex(matrix4f, (float)left, (float)bottom, 0.0F).color(g, h, k, f).next();
        bufferBuilder.vertex(matrix4f, (float)right, (float)bottom, 0.0F).color(g, h, k, f).next();
        bufferBuilder.vertex(matrix4f, (float)right, (float)top, 0.0F).color(g, h, k, f).next();
        bufferBuilder.vertex(matrix4f, (float)left, (float)top, 0.0F).color(g, h, k, f).next();
    }
