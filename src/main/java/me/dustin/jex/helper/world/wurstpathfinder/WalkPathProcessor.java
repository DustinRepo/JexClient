...
public class WalkPathProcessor extends PathProcessor
{
	...
	public void process()
	{
		...
		if(Wrapper.INSTANCE.getPlayer().isOnGround())
			...
			pos = new BlockPos(Wrapper.INSTANCE.getPlayer().position());
		...
		if (WorldHelper.INSTANCE.getBlockState(nextPos).getMaterial().blocksMotion()) {
			Wrapper.INSTANCE.getMultiPlayerGameMode().continueDestroyBlock(nextPos, Direction.UP);
			Wrapper.INSTANCE.getPlayer().swing(InteractionHand.MAIN_HAND);
			return;
		} else if (WorldHelper.INSTANCE.getBlockState(nextPos.above()).getMaterial().blocksMotion()) {
			Wrapper.INSTANCE.getMultiPlayerGameMode().continueDestroyBlock(nextPos.above(), Direction.UP);
			Wrapper.INSTANCE.getPlayer().swing(InteractionHand.MAIN_HAND);
			return;
		}
		...
		if(Feature.getState(Jesus.class)) {
			...
			if(Wrapper.INSTANCE.getPlayer().getY() - nextPos.getY() < -0.5f && (Wrapper.INSTANCE.getPlayer().isInWater() || Wrapper.INSTANCE.getPlayer().isInLava()))
				return;

			// manually swim down if using Jesus
			if(Wrapper.INSTANCE.getPlayer().getY() - nextPos.getY() > 0.5 && (Wrapper.INSTANCE.getPlayer().isInWater() || Wrapper.INSTANCE.getPlayer().isInLava() || WorldHelper.INSTANCE.isOnLiquid(Wrapper.INSTANCE.getPlayer()) || WorldHelper.INSTANCE.isTouchingLiquidBlockSpace(Wrapper.INSTANCE.getPlayer())))
				if (Wrapper.INSTANCE.getPlayer() == Freecam.playerEntity) {
					Wrapper.INSTANCE.getPlayer().setShiftKeyDown(true);
				} else {
					Wrapper.INSTANCE.getOptions().keyShift.setDown(true);
				}
			else
				Wrapper.INSTANCE.getPlayer().setShiftKeyDown(false);
		}
		...
		if(pos.getX() != nextPos.getX() || pos.getZ() != nextPos.getZ())
		{
			...
			if (Speed.INSTANCE.getState()) {
				if (Wrapper.INSTANCE.getPlayer().position().distanceTo(vecInPos) <= getSpeedModSpeed()) {
					PlayerHelper.INSTANCE.setVelocityX(Wrapper.INSTANCE.getPlayer(), 0);
					PlayerHelper.INSTANCE.setVelocityZ(Wrapper.INSTANCE.getPlayer(), 0);
			...
			if(index > 0 && path.get(index - 1).isJumping() || pos.getY() < nextPos.getY()) {
				if (!Feature.getState(Step.class)) {
					double d = (double)(0.42f * getJumpVelocityMultiplier()) + Wrapper.INSTANCE.getPlayer().getJumpBoostPower();
					Vec3 vec3d = Wrapper.INSTANCE.getPlayer().getDeltaMovement();
					if (Wrapper.INSTANCE.getPlayer().isOnGround())
						Wrapper.INSTANCE.getPlayer().setDeltaMovement(vec3d.x, d, vec3d.z);
				}
			}
			// vertical movement
			...
			if(pos.getY() < nextPos.getY()) {
				...
				if(block instanceof LadderBlock || block instanceof VineBlock) {
					Wrapper.INSTANCE.getOptions().keyJump.setDown(true);
				}else {
					// directional jump
				...
				if(Wrapper.INSTANCE.getPlayer().isOnGround()) {
					...
					double newx = -Math.sin(yaw * 3.1415927F / 180.0F) * moveSpeed();
					double newz = Math.cos(yaw * 3.1415927F / 180.0F) * moveSpeed();
					if(Wrapper.INSTANCE.getPlayer().isInWater() || PlayerHelper.INSTANCE.isOnEdgeOfBlock()){
						newx *= 0.4;
						newz *= 0.4;
					}
	...
	protected float getJumpVelocityMultiplier() {
		float f = WorldHelper.INSTANCE.getBlock(Wrapper.INSTANCE.getPlayer().blockPosition()).getJumpFactor();
		float g = WorldHelper.INSTANCE.getBlock(new BlockPos(Wrapper.INSTANCE.getPlayer().position().x, Wrapper.INSTANCE.getPlayer().getBoundingBox().minY - 0.5000001, Wrapper.INSTANCE.getPlayer().position().z)).getJumpFactor();
	...
	{
		double xD = Wrapper.INSTANCE.getPlayer().getX() - pos.x();
		double zD = Wrapper.INSTANCE.getPlayer().getZ() - pos.z();
