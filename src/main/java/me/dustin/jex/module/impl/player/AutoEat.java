...
@ModClass(name = "AutoEat", category = ModCategory.PLAYER, description = "Eat food when hunger is low.")
public class AutoEat extends Module {
    ...
    public void run(Event event) {
        ...
        if (Wrapper.INSTANCE.getLocalPlayer() == null || Module.get(Freecam.class).getState() || Wrapper.INSTANCE.getLocalPlayer().isCreative()) {
            isEating = false;
            return;
        }
    ...
    private boolean needsToEat(FoodInfo foodInfo) {
        if (!eatToRegen) {
            ...
            return 20 - Wrapper.INSTANCE.getLocalPlayer().getHungerManager().getFoodLevel() >= foodInfo.item.getFoodComponent().getHunger() || (Wrapper.INSTANCE.getLocalPlayer().getHealth() < 20 && Wrapper.INSTANCE.getLocalPlayer().getHungerManager().getFoodLevel() < 20);
        }
    }
